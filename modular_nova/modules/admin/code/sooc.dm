...
/proc/toggle_sooc(toggle = null)
	...
	var/list/listeners = list()
	var/static/list/job_lookup = list(JOB_SECURITY_OFFICER = TRUE, JOB_WARDEN = TRUE, JOB_DETECTIVE = TRUE, JOB_HEAD_OF_SECURITY = TRUE, JOB_CAPTAIN = TRUE, JOB_BLUESHIELD = TRUE, JOB_SECURITY_MEDIC = TRUE)
	for(var/iterated_player as anything in GLOB.player_list)
		var/mob/iterated_mob = iterated_player
