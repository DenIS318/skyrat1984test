...
/obj/machinery/status_display/door_timer/proc/timer_end_full(forced = FALSE) // headache of SHOULD_NOT_SLEEP of SPACEMANDMM
	...
	if(R) // update record & status
		...
		var/details_text = "Autogenerated on [current_date] [station_time_timestamp()]. Released for the charges of \"[crimes]\""
...
/obj/machinery/status_display/door_timer/proc/print_report()
	...
	if (SSmachines)
		...
		if (warrant_consoles && length(warrant_consoles) > 0)
			var/paper_name ="[name] log - [prison_occupant] [station_time_timestamp()]"
	...
	if(R) // update record & status
		...
		var/author_name = name
		var/current_date = "[time2text(REALTIMEOFDAY, "MM")]/[time2text(REALTIMEOFDAY, "DD")]/[CURRENT_STATION_YEAR]"
		var/details_text = "Autogenerated on [current_date] [station_time_timestamp()]. Sentenced to [timetoset/10] seconds for the charges of \"[crimes]\" by [rank] [officer]"
		var/fine_amount = -1
	...
	aas_config_announce(/datum/aas_config_entry/brig_cell_incarcerated,
		...
		"USER" = isobserver(usr) ? "Central Command" : officer, "ISSUERRANK" = rank,
		"CRIMES" = crimes),
		src, list(broadcast_channel))
